...
namespace insound {
    struct AudioLoader::Impl {
        explicit Impl(const Engine *engine) : m_targetSpec(), m_threads(4)
        {
        ...
        const SoundBuffer *load(const fs::path &path)
        {
            ...
            uint8_t *buffer;
            uint32_t length;
            if (!loadAudio(path, m_targetSpec, &buffer, &length))
                return nullptr;

            const auto &[emplacedIt, success] = m_buffers.emplace(path.native(), SoundBuffer(buffer, length, m_targetSpec));
            if (!success)
            {
                std::free(buffer);
                return nullptr;
