...
namespace insound {
    inline bool decodeGME(const std::filesystem::path &path, int samplerate, int track, int lengthMS, AudioSpec *outSpec, uint8_t **outData,
      uint32_t *outBufferSize)
    {
        Music_Emu *emu;
        if (const auto result = gme_open_file(path.c_str(), &emu, samplerate); result != nullptr)
        {
